...
def fast_non_dominated_sort(population: List[Individual]) -> List[List[Individual]]:
    """
    Fast non-dominated sorting para classificar indivíduos em frentes de Pareto.
    
    Args:
        population: Lista de indivíduos
        
    Returns:
        Lista de frentes (cada frente é uma lista de indivíduos)
    """
    n = len(population)
    
    # Para cada indivíduo p, armazenar:
    # - S[p]: conjunto de indivíduos dominados por p
    # - n[p]: número de indivíduos que dominam p
    S = [[] for _ in range(n)]
    n_dominated = [0] * n
    ...
    for i in range(n):
        ...
        # Se p não é dominado por ninguém, pertence à primeira frente
        if n_dominated[i] == 0:
            population[i].rank = 0
    ...
    while front_idx < len(fronts) and fronts[front_idx]:
        ...
        for ind in fronts[front_idx]:
            # Encontrar índice do indivíduo na população
            p_idx = population.index(ind)
            
            # Para cada indivíduo q dominado por p
            for q_idx in S[p_idx]:
                n_dominated[q_idx] -= 1
                
                # Se q não é mais dominado por ninguém, vai para a próxima frente
...
def calculate_crowding_distance(front: List[Individual]) -> None:
    ...
    for obj_name in objectives:
        ...
        front_sorted = sorted(front, key=lambda ind: getattr(ind.objectives, obj_name))
        ...
        obj_min = getattr(front_sorted[0].objectives, obj_name)
        obj_max = getattr(front_sorted[-1].objectives, obj_name)
        ...
        for i in range(1, n - 1):
            ...
            obj_prev = getattr(front_sorted[i - 1].objectives, obj_name)
            obj_next = getattr(front_sorted[i + 1].objectives, obj_name)
