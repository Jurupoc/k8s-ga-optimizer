...
class WorkloadProfile:
    ...
    pattern_func: Optional[Callable[[float], int]] = (
        None  # função que retorna concurrency em função do tempo
    )
    ...
    def get_concurrency_at(self, elapsed_time: float) -> int:
        ...
        if elapsed_time < self.ramp_up_duration:
            ...
            return int(
                ...
                + (self.max_concurrency - self.base_concurrency) * progress
            )
        else:
...
def _ramp_up_pattern(
    base: int, max_c: int, ramp_duration: int
) -> Callable[[float], int]:
...
def get_profile(profile_type: str) -> WorkloadProfile:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown profile: {profile_type}. Available: {[p.value for p in LoadProfileType]}"
        )
